...
/*      writetrace - prints the specified 1D trace (or a fraction thereof)
 ...
 Revision history:
   ...
   2010-05-31 - r.kyburz, corrected mis-leading debugging output
...
static char rev[] =     "writetrace.c 3.6";
static char revdate[] = "2011-02-08_19:48";
  ...
  char *argv[];
{
  ...
  if (debug)
  {
    ...
    (void) fprintf(stderr, "    trace#  %12d\n", trace);
    (void) fprintf(stderr, "    offset  %12d\n", offset);
    if (points == 1073741824)
    {
      if (offset == 0)
        (void) fprintf(stderr, "    points  %12s\n", "(all)");
      else
        (void) fprintf(stderr, "    points  %12s\n", "(remaining)");
    }
    else
      (void) fprintf(stderr, "    points  %12d\n", points);
  }
